...
import { LigneStation } from '../components/stationComponent.ts';
...
class StationApi {
    ...
    async obtenirStationsDirect(idLigne: number): Promise<LigneStation[]> {
        try {
            let reponse = await fetch(`${URL_API}/ligneStations/ligne/${idLigne}/asc`);
            const premierEssaiOK = reponse.ok;
            if (!premierEssaiOK) {
                reponse = await fetch(`${URL_API}/ligneStations/ligne/${idLigne}`);
            }
    ...
    async obtenirStationsIndirect(idLigne: number): Promise<LigneStation[]> {
        try {
            let reponse = await fetch(`${URL_API}/ligneStations/ligne/${idLigne}/desc`);
            const premierEssaiOK = reponse.ok;
            if (!premierEssaiOK) {
                reponse = await fetch(`${URL_API}/ligneStations/ligne/${idLigne}`);
            }
